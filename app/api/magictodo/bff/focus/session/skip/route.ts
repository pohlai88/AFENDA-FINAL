/**
 * Magictodo BFF - Focus session skip task
 * POST: Skip the current task in the active focus session and advance to next
 *
 * @domain magictodo
 * @layer api/bff
...
type DbParam = Parameters<typeof magictodoFocusService.skipTask>[4];

/**
 * POST /api/magictodo/bff/focus/session/skip
 * Skip current focus task and advance to next in queue.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await magictodoFocusService.skipTask(
      userId,
      body.sessionId,
      organizationId,
      teamId,
    ...
    return NextResponse.json(
      kernelFail(
        {
          code: KERNEL_ERROR_CODES.INTERNAL,
          message: "Failed to skip focus task",
