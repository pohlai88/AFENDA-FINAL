...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const org = await tenancyOrganizationService.getById(id);
    if (!org) {
      return NextResponse.json(
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const org = await tenancyOrganizationService.update(id, body);
    if (!org) {
      return NextResponse.json(
        kernelFail({ code: KERNEL_ERROR_CODES.NOT_FOUND, message: "Organization not found" }, { traceId }),
        { status: HTTP_STATUS.NOT_FOUND, headers: { [KERNEL_HEADERS.REQUEST_ID]: traceId, [KERNEL_HEADERS.TRACE_ID]: traceId } }
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const deleted = await tenancyOrganizationService.delete(id);
    if (!deleted) {
      return NextResponse.json(
        kernelFail({ code: KERNEL_ERROR_CODES.NOT_FOUND, message: "Organization not found" }, { traceId }),
        { status: HTTP_STATUS.NOT_FOUND, headers: { [KERNEL_HEADERS.REQUEST_ID]: traceId, [KERNEL_HEADERS.TRACE_ID]: traceId } }
